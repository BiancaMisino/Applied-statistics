...
##TOLGO DIAM IN MM E INTERVAL GREATER THAN 30
...
for (j in 1:5) {
  ###CAMBIA MODELLO OGNI VOLTA
  model<-glm(TRG ~ CEA + ANTI_VEGF + OXALIPLATINO + Linee_di_chemioterapia + 
               IRINOTECAN  ,
...
for (j in 1:5) {
  ###CAMBIA MODELLO OGNI VOLTA
  model<-glm(TRG ~ ANTI_VEGF + primary_tumor_site_left
               ,
...
for (j in 1:5) {
  ###CAMBIA MODELLO OGNI VOLTA
  model<-glm(TRG ~ Diametro_in_mm + ANTI_VEGF + primary_tumor_site_left
             + Linee_di_chemioterapia+sync+bilater+greater_6_cicli   ,
...
for (j in 1:5) {
  ###CAMBIA MODELLO OGNI VOLTA  
  model<-glm(TRG ~ Risposta_radiologica + ANTI_VEGF + CEA + SHAPE_Volume..mL. + 
               Diametro_in_mm + sync  + primary_tumor_site_left  
                 ,
             data=DB_cliniche_casoa[folds!=j,], family=binomial)
  probabilities <- predict(model,newdata=DB_cliniche_casoa[folds==j,],type = 'response')
  #andrebbe messo la frequenza di trg=1
...
for (j in 1:5) {
  ###CAMBIA MODELLO OGNI VOLTA
  model<-glm(TRG ~ CEA + ANTI_VEGF + OXALIPLATINO + Linee_di_chemioterapia + 
               GLRLM_SRLGE + Interval._greater_30.days + IRINOTECAN + Diametro_in_mm + 
               NGLDM_Busyness   ,
             data=DB_cliniche_senzarispradA[folds!=j,], family=binomial)
  probabilities <- predict(model,newdata=DB_cliniche_senzarispradA[folds==j,],type = 'response')
  #andrebbe messo la frequenza di trg=1
...
for (j in 1:5) {
  ###CAMBIA MODELLO OGNI VOLTA
  model<-glm(TRG ~ GLZLM_LGZE + primary_tumor_site_left + 
               GLZLM_ZP   ,
             data=DB_cliniche_casob[folds!=j,], family=binomial)
  probabilities <- predict(model,newdata=DB_cliniche_casob[folds==j,],type = 'response')
  #andrebbe messo la frequenza di trg=1
...
vif(best.fit)
summary(best.fit)
##TOLGO GREATER DI 6 CICLI , DIAM IN MM
...
for (j in 1:5) {
  ...
  model<-glm(TRG ~ GLRLM_SRLGE + primary_tumor_site_left + Linee_di_chemioterapia  
                + OXALIPLATINO + GLCM_Homogeneity...Inverse.difference. + 
               IRINOTECAN   ,
             data=DB_cliniche_senzarispradB[folds!=j,], family=binomial)
  probabilities <- predict(model,newdata=DB_cliniche_senzarispradB[folds==j,],type = 'response')
  #andrebbe messo la frequenza di trg=1
...
vif(best.fit)
summary(best.fit)
##TOLGO OXALIPLATINO, M_CLZLM_GLNU
...
for (j in 1:5) {
  ###CAMBIA MODELLO OGNI VOLTA
  model<-glm(TRG ~ Risposta_radiologica + ANTI_VEGF + CEA + M_SHAPE_Sphericity..only.for.3D.ROI..nz.1. + 
               M_HISTO_Entropy_log2 + GLRLM_SRLGE + M_GLRLM_LGRE + M_CONVENTIONAL_HUmean  
                + M_GLZLM_SZE  + Diametro_in_mm + 
               numerometastasi_2to3 + SHAPE_Compacity.only.for.3D.ROI..nz.1.  ,
             data=DB_cliniche_casoa[folds!=j,], family=binomial)
  probabilities <- predict(model,newdata=DB_cliniche_casoa[folds==j,],type = 'response')
  #andrebbe messo la frequenza di trg=1
  predicted <- ifelse(probabilities > 0.68, "1","0")
...
for (j in 1:5) {
  ...
  model<-glm(TRG ~ CEA + ANTI_VEGF + M_SHAPE_Sphericity..only.for.3D.ROI..nz.1. + 
               sync + Diametro_in_mm + GLRLM_SRLGE + M_GLZLM_SZE + SHAPE_Volume..mL. + 
               GLRLM_GLNU + primary_tumor_site_left   ,
             data=DB_cliniche_senzarispradA[folds!=j,], family=binomial)
  probabilities <- predict(model,newdata=DB_cliniche_senzarispradA[folds==j,],type = 'response')
  #andrebbe messo la frequenza di trg=1
  predicted <- ifelse(probabilities > 0.68, "1","0")
...
vif(best.fit)
summary(best.fit)
##TOLGO RISP RAD (ENORME),LINEE DI CHEMIO E CONTRASTO
...
for (j in 1:5) {
  ###CAMBIA MODELLO OGNI VOLTA
  model<-glm(TRG ~  GLZLM_LGZE + primary_tumor_site_left + 
               M_CONVENTIONAL_HUstd + CONVENTIONAL_HUstd   
                 ,
             data=DB_cliniche_casob[folds!=j,], family=binomial)
  probabilities <- predict(model,newdata=DB_cliniche_casob[folds==j,],type = 'response')
  #andrebbe messo la frequenza di trg=1
...
vif(best.fit)
summary(best.fit)
##TOLGO GLRLM_SRLGE,LINEE DI CHEMIO , GLZLM_LGZE,DIAM IN MM 
...
for (j in 1:5) {
  ###CAMBIA MODELLO OGNI VOLTA
  model<-glm(TRG ~  primary_tumor_site_left +  
               greater_6_cicli + M_GLZLM_SZE  + sync   ,
             data=DB_cliniche_senzarispradB[folds!=j,], family=binomial)
  probabilities <- predict(model,newdata=DB_cliniche_senzarispradB[folds==j,],type = 'response')
  #andrebbe messo la frequenza di trg=1
  predicted <- ifelse(probabilities > 0.86, "1","0")
...
best.fit$coefficients
###CAMBIA MODELLO OGNI VOLTA
model<-glm(TRG ~  primary_tumor_site_left + Linee_di_chemioterapia + 
             greater_6_cicli + M_GLZLM_SZE  + sync    ,
           data=DB_cliniche_senzarispradB, family=binomial)
