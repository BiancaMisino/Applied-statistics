...
colonne_con_na<-subset(DB_cliniche[,c(8,9,10)])
...
for (j in 1:5) {
  ...
  model<-glm(TRG ~ Risposta_radiologica + ANTI_VEGF + CEA+BRAF
             + Linee_di_chemioterapia+OXALIPLATINO+IRINOTECAN  ,
...
for (j in 1:5) {
  ###CAMBIA MODELLO OGNI VOLTA
  model<-glm(TRG ~ CEA + ANTI_VEGF + OXALIPLATINO + BRAF+Linee_di_chemioterapia + 
               IRINOTECAN + `Interval _greater_30 days`  ,
...
for (j in 1:5) {
  ###CAMBIA MODELLO OGNI VOLTA
  model<-glm(TRG ~ primary_tumor_site_left+BRAF+ ANTI_VEGF   
               ,
...
for (j in 1:5) {
  ...
  model<-glm(TRG ~  ANTI_VEGF + primary_tumor_site_left
             + Linee_di_chemioterapia+BRAF+OXALIPLATINO+IRINOTECAN  ,
...
for (j in 1:5) {
  ###CAMBIA MODELLO OGNI VOLTA  
  model<-glm(TRG ~ Risposta_radiologica + ANTI_VEGF + CEA +BRAF+ SHAPE_Volume..mL. + 
               Diametro_in_mm + `GLCM_Homogeneity (=Inverse difference)`   
                 ,
             data=DB_cliniche_casoa[folds!=j,], family=binomial)
  probabilities <- predict(model,newdata=DB_cliniche_casoa[folds==j,],type = 'response')
  #andrebbe messo la frequenza di trg=1
...
for (j in 1:5) {
  ...
  model<-glm(TRG ~ CEA + ANTI_VEGF + OXALIPLATINO + BRAF+ Linee_di_chemioterapia + 
               ...
               NGLDM_Busyness+ `GLCM_Entropy_log2 (=Joint entropy)`   ,
             data=DB_cliniche_senzarispradA[folds!=j,], family=binomial)
  probabilities <- predict(model,newdata=DB_cliniche_senzarispradA[folds==j,],type = 'response')
  #andrebbe messo la frequenza di trg=1
  predicted <- ifelse(probabilities > 0.68, "1","0")
...
for (j in 1:5) {
  ###CAMBIA MODELLO OGNI VOLTA
  model<-glm(TRG ~ GLZLM_LGZE + primary_tumor_site_left +BRAF+ 
               GLZLM_ZP +ANTI_VEGF  ,
             data=DB_cliniche_casob[folds!=j,], family=binomial)
  probabilities <- predict(model,newdata=DB_cliniche_casob[folds==j,],type = 'response')
  #andrebbe messo la frequenza di trg=1
...
for (j in 1:5) {
  ###CAMBIA MODELLO OGNI VOLTA
  model<-glm(TRG ~ GLRLM_SRLGE + BRAF +primary_tumor_site_left + Linee_di_chemioterapia  
                + OXALIPLATINO  + ANTI_VEGF
               IRINOTECAN   ,
             data=DB_cliniche_senzarispradB[folds!=j,], family=binomial)
  probabilities <- predict(model,newdata=DB_cliniche_senzarispradB[folds==j,],type = 'response')
  #andrebbe messo la frequenza di trg=1
...
for (j in 1:5) {
  ...
  model<-glm(TRG ~ CEA + ANTI_VEGF + M_SHAPE_Sphericity..only.for.3D.ROI..nz.1. + 
               BRAF +OXALIPLATINO+ Diametro_in_mm SHAPE_Volume..mL. + 
               IRINOTECAN   ,
             data=DB_cliniche_senzarispradA[folds!=j,], family=binomial)
  probabilities <- predict(model,newdata=DB_cliniche_senzarispradA[folds==j,],type = 'response')
  #andrebbe messo la frequenza di trg=1
...
for (j in 1:5) {
  ###CAMBIA MODELLO OGNI VOLTA
  model<-glm(TRG ~  primary_tumor_site_left +  BRAF
               + GLRLM_SRLGE+ANTI_VEGF+Linee_di_chemioterapia+OXALIPLATINO  ,
             data=DB_cliniche_senzarispradB[folds!=j,], family=binomial)
  probabilities <- predict(model,newdata=DB_cliniche_senzarispradB[folds==j,],type = 'response')
  #andrebbe messo la frequenza di trg=1
  predicted <- ifelse(probabilities > 0.86, "1","0")
